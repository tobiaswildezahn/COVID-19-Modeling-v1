...
from chime_develop.src.penn_chime.models import  SimSirModel as Model
from chime_develop.src.penn_chime.parameters import Parameters
from chime_develop.src.penn_chime.parameters import Disposition as RateDays
    ...
    for varName, field, constVal in zip(colNames, fieldArray, constantArray):
        if field:
            ...
            if constVal is None:
                ...
                raise SystemExit()
            out_df[varName] = np.tile(float(constVal), n)
        else:
...
def updateArray(arrays,index,slc, df, outputFields, typeOutputFields):
...
def overCapStats(df, index, ssdo, fieldName,refNameDF, duration ):
    ...
    return maxH-value, reachMax, dMax, prc*100, numberDaysMax
...
def checkValue(parameter,  minValue = 0 , maxValue = 100, onlyCheckLowerBound = False):
    if parameter.value:
...
def getAliasFields(fieldNames):
    alias = {'DoublingTime' : 'Doubing Time',
     'SocDistPerc' : 'Social Distance %',
     'InfectedHospPerc' : 'Infected Hospitalized %',
     'InfectedICUPerc' : 'Infected ICU %',
     'InfectedVentPerc' : 'Infected Ventilator %',
     'HospitalStay' : 'Hospital Stay',
     'ICU_Stay' : 'ICU Stay',
     'Vent_Stay' : 'Ventilator Stay',
     'HospMarketSharePerc' : 'Hospital Market Share %',
     'InfectiousDays' : 'Infectious Days',
     'Num_Hospitalized' : 'Number of Hospitalized',
     'Num_Infections' : 'Number of Infections',
...
class CHIME(object):
    def __init__(self):
        ...
        self.label = "CHIME Model v1.1.1"
        self.description = "Generate time enabled point feature class showing hospital bed demand over time"
    ...
    def getParameterInfo(self):
        ...
        param2.filter.list = ['Short', 'Long', 'Double']
        ...
        param14.filter.list = ['Short', 'Long']
        ...
        param16 = arcpy.Parameter(displayName = "Start Date",
                                 ...
                                 parameterType = "Required",
                                 direction = "Input")
        param16.value = date.today().strftime("%m/%d/%Y")
        ...
        param27.filter.list = ['Short', 'Long']
        ...
        param28.filter.list = ['Short', 'Long']
        ...
        param29.filter.list = ['Short', 'Long']
    ...
    def updateMessages(self, parameters):
        ...
        checkValue(parameters[17], minValue= 0.5, onlyCheckLowerBound = True)
    ...
    def execute(self, parameters, messages):
        ...
        start_time = utils.getTextParameter(16, parameters)
        ...
        fieldNames = ['DoublingTime', 'SocDist%', 'InfectedHosp%', 'InfectedICU%', 'InfectedVent%',
                      'HospitalStay', 'ICU_Stay', 'Vent_Stay', 'HospMarketShare%', 'InfectiousDays']
        ...
        data_mat["Num_Hospitalized"] = ssdo.fields[allInFields[2].upper()].data

        diff = data_mat["Population"] - data_mat["Num_Hospitalized"]
        if (diff <= 0).sum() >=1:
        ...
        listOther = ["Population", "Num_Hospitalized"]

        # Epsilon Zeros for NUmber Hospitalizeds
        data_mat.loc[data_mat["Num_Hospitalized"] == 0, "Num_Hospitalized"] = 1e-06
        ...
        if allInFields[1]:
            data_mat["Num_Infections"] = ssdo.fields[allInFields[1].upper()].data
            listOther = ["Population", "Num_Hospitalized","Num_Infections"]
        ...
        ## PARAMETER CHECKS
        if (data_mat['DoublingTime'] > duration).any():
            arcpy.AddMessage('Doubling time cannot be greater than model duration')
            raise SystemExit()
        elif (data_mat['DoublingTime'] < 0.5).any():
            arcpy.AddMessage('Doubling time cannot be less than 0.5')
            raise SystemExit()
        if ((data_mat['SocDist%'] < 0).any() or (data_mat['SocDist%'] > 100).any()):
            arcpy.AddMessage('Social distancing % (Reduction in Social Contact Going Forward) must be between 0 and 100')
            raise SystemExit()
        if (data_mat['InfectedHosp%'] < 0).any() or (data_mat['InfectedHosp%'] > 100).any():
            arcpy.AddMessage('Hospitalization % (Total Infections) must be between 0 and 100')
            raise SystemExit()
        if ((data_mat[ 'InfectedICU%'] < 0).any() or (data_mat['InfectedICU%'] > 100).any()):
            arcpy.AddMessage('ICU % (Total Infections) must be between 0 and 100')
            raise SystemExit()
        if ((data_mat[ 'InfectedVent%'] < 0).any() or (data_mat['InfectedVent%'] > 100).any()):
            arcpy.AddMessage('Ventilated % (Total Infections) must be between 0 and 100')
            raise SystemExit()
        if (data_mat['HospitalStay'] < 0 ).any():
            arcpy.AddMessage('Average Hospital Length of Stay (days) must be greater than 0')
            raise SystemExit()
        if (data_mat['ICU_Stay'] < 0 ).any():
            arcpy.AddMessage('Average days in ICU must be greater than 0')
            raise SystemExit()
        if (data_mat['Vent_Stay'] < 0 ).any():
            arcpy.AddMessage('Average Days on Ventilator must be greater than 0')
            raise SystemExit()
        if (data_mat['InfectiousDays'] < 0 ).any():
            arcpy.AddMessage('Infectious days must be greater than 0')
            raise SystemExit()
        ...
        for id, loc in data_mat.iterrows():
            chime_param = Parameters(population = int(loc["Population"]),
                                     doubling_time = loc["DoublingTime"],
                                     relative_contact_rate = loc["SocDist%"]/100,
                                     current_hospitalized = loc["Num_Hospitalized"],
                                     hospitalized=RateDays(loc['InfectedHosp%']/100, int(loc['HospitalStay'])),
                                     icu=RateDays(loc['InfectedICU%']/100, int(loc['ICU_Stay'])),
                                     ventilated=RateDays(loc['InfectedVent%']/100, int(loc['Vent_Stay'])),
                                     infectious_days = loc["InfectiousDays"],
                                     date_first_hospitalized = None,
                                     n_days= duration,
                                     current_date=dt.strptime(start_time, "%m/%d/%Y"))
            ...
            df = {}
            for i in  [ 'day', 'date', 'hospitalized', 'icu', 'ventilated']:
            ...
            n = len(loc)
            df_census = {}
            for i in  [ 'hospitalized', 'icu', 'ventilated']:
                val = model.census_df[i].values.copy()
                df_census[i+"_census"] = val[maskV]
            ...
            index = 0
            index = updateArray(arrays, index, slc, df, outputFields, typeOutputFields )
            index = updateArray(arrays, index, slc, df_census, outputFieldsCensus, typeOutputFieldsCensus)
            index = updateArray(arrays, index, slc, df_infect, outputFieldsInfect, typeOutputFieldsInfect)
            ...
            #### Add Statistics ####
            information = basicStats(df_census, nameH="hospitalized_census", nameI = "icu_census", nameV = "ventilated_census")
            ...
            if dictOpt["hos"]is not None:
                information = overCapStats(df_census, id, ssdo, dictOpt["hos"], 'hospitalized_census', duration)
                for valStat in information:
                    index2d+=1
                    arrays2d[index2d][id] =  valStat

            if dictOpt["icu"] is not None:
                information = overCapStats(df_census, id, ssdo, dictOpt["icu"], 'icu_census', duration)
            ...
            if dictOpt["vnt"] is not None:
                information = overCapStats(df_census, id, ssdo, dictOpt["vnt"], 'ventilated_census', duration)
                for valStat in information:
                    index2d+=1
                    arrays2d[index2d][id] =  valStat
        ...
        ARCPY.SetProgressor("default", "Writing outputs feature classes...")
        if getGeometry:
            #pdb.set_trace()
            shapes = []
            ...
            container = utils.DataContainer(spatialRef = ssdo.spatialRef, shapes = shapes, z = zValues)
            ...
            container = utils.DataContainer(spatialRef = ssdo.spatialRef, xy = np.tile(ssdo.xyCoords, duration).reshape(n*duration,2), z = zValues)
            fields3d = fieldsNameOutput + ["SOURCE_ID"]
            fields3dAliases = getAliasFields(fields3d)
            container.generateOutput(output_fc, arrays, fields3d, fields3dAliases)
        ...
        chart3.yAxis.field = ["susceptible", "infected", "recovered"]
        ...
        if output_fc2d not in ["",None]:
            ...
            if getGeometry:
                ...
                if ssdo.hasZ:
                    zValues = ssdo.zCoords
                container = utils.DataContainer(spatialRef = ssdo.spatialRef, shapes = list(ssdo.shapes), z = zValues)
                ...
                container = utils.DataContainer(spatialRef = ssdo.spatialRef, xy = ssdo.xyCoords,z = zValues)
